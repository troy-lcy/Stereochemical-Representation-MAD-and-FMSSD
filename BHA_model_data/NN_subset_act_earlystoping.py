...
# Early Stopping 参数
patience = 20  # 容忍的 epoch 数
best_r2 = float('-inf')  # 初始化最好的 r2_score
epochs_since_best = 0  # 记录自上次最佳 r2_score 以来的 epoch 数

for times in range(10): # 重复十次
    for epoch in range(EPOCHS):
        ...
        r2_test = []
        with torch.no_grad():
            for batch_idx, (data, target) in enumerate(Valid_loader[2]):
                if batch_idx * len(data) >= len(data):
                    break
                data, target = data.to(DEVICE), target.to(DEVICE)
                output = model(data.to(torch.float))
        ...
        if r2_test > best_r2:
            best_r2 = r2_test
            epochs_since_best = 0  # Reset counter
            # print(f'yes, {best_r2}')
            ...
            if epochs_since_best >= patience:
                ...
                best_total.append(best_r2)

                # np.savetxt('MAD_additive_best_r2_'+ 'test3.csv',best_total,delimiter=',')
                # np.savetxt('MAD_additive_test_target_pred_'+ 'test3.csv',target_pred_test,delimiter=',')
                # np.savetxt('MAD_additive_r2_'+ 'test3.csv', r2_test_total,delimiter=',')               
                break  # 提前停止训练
        ...
        if (epoch+1) == EPOCHS:
                ...
                # np.savetxt('MAD_additive_best_r2.csv',best_total,delimiter=',')
                # np.savetxt('MAD_additive_test_target_pred_'+  'test3.csv',target_pred_test,delimiter=',')
                # np.savetxt('MAD_additive_r2_'+ 'test3.csv', r2_test_total,delimiter=',')      
np.savetxt('MAD_additive_best_r2_'+ 'test3.csv',best_total,delimiter=',')
